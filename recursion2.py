...
#  Description:Set of Recursion problems
...
def splitOdd10Helper(nums, i, mult10, odd):
	if i >= len(nums):
		return mult10 % 10 == 0 and odd % 2 == 1
	if splitOdd10Helper(nums, i + 1, mult10 + nums[i], odd):
		return True
	if splitOdd10Helper(nums, i + 1, mult10, odd + nums[i]):
		return True
	return False
...
def split53Helper(nums, i, balance):
	if i == len(nums):
		return balance == 0
	if nums[i] % 5 == 0:
